    ...
    <>
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat
          ...
          value={kpis.avgDurationDays != null ? fmtDuration(kpis.avgDurationDays) : "—"}
          provenance="live"
          sub={kpis.avgDurationDays != null ? "issue → completion" : "modeled by type"}
        />
      </section>
        ...
        <div className="flex flex-col gap-4">
          ...
          {demand.available && (
            <Card className="flex flex-col gap-2">
              <div className="text-xs text-muted">Demand context (Census ACS)</div>
              <Row label="Population" value={fmtNum(demand.population)} />
              <Row label="Pop. growth (5yr)" value={demand.popGrowthPct != null ? `${demand.popGrowthPct >= 0 ? "+" : ""}${demand.popGrowthPct.toFixed(1)}%` : "—"} />
              <Row label="Vacancy" value={demand.vacancyPct != null ? `${demand.vacancyPct.toFixed(1)}%` : "—"} />
              <Row label="Renter share" value={demand.renterPct != null ? `${demand.renterPct.toFixed(0)}%` : "—"} />
              <Row label="Median rent" value={demand.medianRent != null ? `$${fmtNum(demand.medianRent)}` : "—"} />
            </Card>
          )}
        </div>
      </section>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between">
      ...
      <span className="font-display text-[15px] font-semibold num">{value}</span>
    </div>
  );
}
